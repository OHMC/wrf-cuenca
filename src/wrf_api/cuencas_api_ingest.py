...
def create_wrf_object(api_base_url, token, nombre, payload: dict):
    ...
    try:
        r = requests.post(f"{api_base_url}/{nombre}/", json=payload,
                          headers=headers_base)
        r.raise_for_status()
        wrf_id = r.json().get('id')
        return wrf_id
    except RequestException:
        logger.exception(f"Error al crear objeto - nombre={nombre},"
                         f"payload={payload}")
        raise
...
def ingest_csv_to_db(cuencas_dict: dict):
    ...
    for api_base_url, meta in API_BASE_URL_DICT.items():
        try:
            parametrizacion_id = get_wrf_api_object_id(api_base_url,
                                                       'parametrizacion',
                                                       query=f"?nombre={cuencas_dict['meta']['param']}")
        except Exception:
        ...
        for prod in cuencas_dict['csv'].keys():
            ...
            try:
                producto_cuenca_id = get_wrf_api_object_id(api_base_url,
                                                           'producto-cuencas',
            ...
            try:
                create_wrf_object(api_base_url,
                                  meta['token'],
                                  nombre='csvs-cuencas',
                                  payload=payload)
            except Exception:
