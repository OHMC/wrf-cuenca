...
def guardar_tabla(cuencas_gdf_ppn, outdir, rundate, configuracion):
    path = outdir + rundate.strftime('%Y_%m/%d/cordoba/') + 'cuencas_' + configuracion + '.csv'
    try:
        os.makedirs(os.path.dirname(path))
    except OSError:
        pass
...
def main():
	usage = """cuencas.py [--wrfout=pathToDatosWRF] [--outdir_productos=..] [--outdir_tabla ..] [--configuracion ..]"""
	parser = OptionParser(usage)

	parser.add_option("--wrfout",dest="wrfout",help="ruta al wrfout de la salida del WRF")
	parser.add_option("--outdir_productos",dest="outdir_productos",help="ruta donde se guardan los productos")
	parser.add_option("--outdir_tabla",dest="outdir_tabla",help="ruta donde se guardan las tablas de datos")
	parser.add_option("--configuracion",dest="configuracion",help="configuracion de las parametrizaciones")


	(opts,args) = parser.parse_args()
	if not opts.wrfout or not opts.outdir_tabla or not opts.outdir_productos or not opts.configuracion:
